...
- [ ] **Step 3: 实现 `SyncProgressTracker`（GREEN）**
...
- [ ] **Step 4: 重构 `sync_historical_for_pool`**
...
- [ ] **Step 6: 改 `main.py:46-52` startup hook**
...
- [ ] **Step 7: 跑全部 daily_sync 测试验证 GREEN**
...
- [ ] **Step 3: 改 `backend/app/schemas.py`**
...
- [ ] **Step 4: 改 `backend/app/api/sync.py`**
...
- [ ] **Step 4: 同步更新 `DynamicPoolPage.test.tsx` 中所有 `syncHistory.mutate()` 调用为 `syncHistory.mutate({ from_date, to_date })`**
...
- [ ] **Step 1: 在 `frontend/src/api/hooks.ts` 中 export `ProgressInfo` TS 类型**
...
- [ ] **Step 2: 写 `SyncProgressBanner` 测试（RED）**
...
- [ ] **Step 4: 写 `RowProgressBar` 测试（RED）+ 实现**
...
- [ ] **Step 6: 写 `DynamicPoolPage.test.tsx` 进度相关测试（RED）**
...
- [ ] **Step 4: 跑 final review（subagent-driven-development）**
